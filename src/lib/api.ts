...
export const api = {
  ...
  movies: {
    getTrending: (page?: number) => tmdb.isConfigured() 
      ? tmdb.getTrending(page) 
      : request<Movie[]>("/movies/trending").catch(() => tmdb.getTrending(page)),
    getTopRated: (page?: number) => tmdb.isConfigured() 
      ? tmdb.getTopRated(page) 
      : request<Movie[]>("/movies/top-rated").catch(() => tmdb.getTopRated(page)),
    getUpcoming: (page?: number) => tmdb.isConfigured() 
      ? tmdb.getUpcoming(page) 
      : request<Movie[]>("/movies/upcoming").catch(() => tmdb.getUpcoming(page)),
    search: (query: string, page?: number) => tmdb.isConfigured() 
      ? tmdb.search(query, page) 
      : request<Movie[]>(`/movies/search?q=${encodeURIComponent(query)}&page=${page || 1}`).catch(() => tmdb.search(query, page)),
    getDetails: (id: string) => tmdb.isConfigured() 
      ? tmdb.getDetails(id) 
      : request<Movie>(`/movies/${id}`).catch(() => tmdb.getDetails(id)),
    getSimilar: (id: string) => tmdb.isConfigured() 
      ? tmdb.getSimilar(id) 
      : request<Movie[]>(`/movies/${id}/similar`).catch(() => tmdb.getSimilar(id)),
    getRecommendations: (id: string) => tmdb.isConfigured() 
      ? tmdb.getRecommendations(id) 
      : request<Movie[]>(`/movies/${id}/similar`).catch(() => tmdb.getRecommendations(id)),
    getByGenre: (genreId: number, page?: number) => tmdb.isConfigured()
      ? tmdb.getByGenre(genreId, page)
      : tmdb.getByGenre(genreId, page),
    getBollywood: (page?: number) => tmdb.isConfigured()
      ? tmdb.getBollywood(page)
      : tmdb.getBollywood(page),
    getKorean: (page?: number) => tmdb.isConfigured()
      ? tmdb.getKorean(page)
      : tmdb.getKorean(page),
    getAnime: (page?: number) => tmdb.isConfigured()
      ? tmdb.getAnime(page)
      : tmdb.getAnime(page),
  },
  ...
  // AI Assistant
  ai: {
    recommend: (prompt: string) => request<{ message: string; recommendations: Movie[] }>("/ai/recommend", {
      method: "POST",
